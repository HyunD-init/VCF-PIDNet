...
if __name__ == "__main__":
    ...
    for orig_type, img_dataset in orig_data.items():
        ...
        for training_mode, img_paths in img_dataset.items():
            ...
            dst_mask_direc = os.path.join(data_root_path, 'won_dataset', training_mode, 'masks')
            dst_img_direc = os.path.join(data_root_path, 'won_dataset', training_mode, 'images')
            os.makedirs(dst_mask_direc, exist_ok=True)
            ...
            for img_src_path in tqdm(img_paths):
                ...
                convert_colored_mask(json_src_path, dst_mask_direc, vcf_Label_Class, vcf_normal_class)
                convert_colored_mask(json_src_path, dst_mask_direc, Label_Class)